...
class Barcode {
	...
	private $styeleCode 	= "<style>div.b128{ border-left: 1px black solid; height: 60px;}</style>";
	...
	public function set_bar128($text=FALSE)
	{
		
		$w 		= $this->char128wid[$sum = 104];						
		$onChar	= 1;
		
		for($x=0;$x<strlen($text);$x++)								
			if (!( ($pos = strpos($this->char128asc,$text[$x])) === false )){
				$w.= $this->char128wid[$pos];
				$sum += $onChar++ * $pos;
			}					
