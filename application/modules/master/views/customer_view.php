	...
	<div class="navbar-left">
		<div class="btn-group">
			...
			echo'<button class="btn btn-success navbar-btn " id="add-customer-'.$timestamp.'">
						<i class="glyphicon glyphicon-plus"></i> '.lang('btn_cst_add').'
				</button> ';
			echo '<button class="btn btn-info navbar-btn " id="edit-'.$timestamp.'" ><i class="glyphicon glyphicon-pencil"></i> '.lang('btn_cst_edit').'</button> ';
			echo '<button class="btn btn-danger navbar-btn " id="delete-'.$timestamp.'"><i class="glyphicon glyphicon-remove"></i> '.lang('btn_cst_del').' </button> ';
			?>
		</div>
	</div>
...
<div id="Modal-customer-<?php echo $timestamp; ?>" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form class="form-horizontal" id='form-customer-<?php echo $timestamp; ?>' action='<?php echo site_url('master/customer/proses'); ?>' method='POST'>
			<div class="modal-body">
			<fieldset>
					<legend><?php echo lang('form_cst_blok_crp'); ?> :</legend>
					<div class="form-group" id='text_id'>
						<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_id'); ?> :</label>
						<div class="col-sm-9">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label" for=""> <?php echo lang('form_cst_name'); ?> :</label>
						<div class="col-sm-9">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_adres'); ?> :</label>
						<div class="col-sm-9">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_country'); ?> :</label>
						<div class="col-sm-9">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_type'); ?> :</label>
						<div class="col-sm-9">
							 <input name='cust_type' class='kendodropdown form-control' style="width: 100%;" />
						</div>
					</div>
					
					<div class="form-group">
						<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_npwp'); ?> :</label>
						<div class="col-sm-9">
				...
				<fieldset>
					<legend><?php echo lang('form_cst_blok_cp'); ?> :</legend>
						<div class="form-group">
							<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_sal'); ?> :</label>
							<div class="col-sm-9">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_frst_name'); ?>:</label>
							<div class="col-sm-9">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_last_name'); ?>:</label>
							<div class="col-sm-9">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_phone'); ?>:</label>
							<div class="col-sm-9">
								<input type='text' name='phone' class='form-control' onkeypress='numberOnly(event)'/>
							</div>
						</div>

						<div class="form-group">
							<label class="col-sm-3 control-label" for=""><?php echo lang('form_cst_email'); ?>:</label>
							<div class="col-sm-9">
...
$(document).ready(function(){
	...
	var myCmb={
		...
		'cust_type':{url:'<?php echo site_url('app/cust_type');?>',data:{}},
	},cmb={};
		...
		$.ajax({
			...
			success:function(res){
				if(res.status){
					...
					$('#form-customer-<?php echo $timestamp; ?> input[name=first_name]').val('');
					$('#form-customer-<?php echo $timestamp; ?> input[name=last_name]').val('');
					$('#form-customer-<?php echo $timestamp; ?> input[name=phone]').val('');
					$('#form-customer-<?php echo $timestamp; ?> input[name=email]').val('');
					$('#form-customer-<?php echo $timestamp; ?> input[name=npwp]').val('');
					$('#form-customer-<?php echo $timestamp; ?> input[name=cust_type]').val('');
					$('#form-customer-<?php echo $timestamp; ?> textarea[name=alamat]').val('');
				}else
	...
	$('#delete-<?php echo $timestamp; ?>').click(function(){
		...
		if(checkedVals !=''){
			var r = confirm("Hapus data Customer Id ("+checkedVals+")...?");
			if (r == true) {
				checkedVals.forEach(function(element) {
					$("#"+element).hide('slow');
				});
				var customerId= checkedVals.join(",");
				$.post('<?php echo site_url('master/customer/hapus'); ?>',{id:customerId},function(){
					ds_customer.read();
				});
			}
		}else
		...
		if(checkedVals.length ==1){
			...
			$.ajax({
				...
				success:function(res){
					...
					$('#form-customer-<?php echo $timestamp; ?> input[name=nama]').val(res.data[0].customer);
					$('#form-customer-<?php echo $timestamp; ?> input[name=first_name]').val(ar_name[1]);
					$('#form-customer-<?php echo $timestamp; ?> input[name=last_name]').val(ar_name[2]);
					$('#form-customer-<?php echo $timestamp; ?> input[name=phone]').val(res.data[0].phone);
					$('#form-customer-<?php echo $timestamp; ?> input[name=email]').val(res.data[0].email);
					$('#form-customer-<?php echo $timestamp; ?> input[name=npwp]').val(res.data[0].npwp);
					$('#form-customer-<?php echo $timestamp; ?> textarea[name=alamat]').val(res.data[0].alamat);
					cmb['country'].value(res.data[0].country);
					cmb['salutatsion'].value(ar_name[0]);
					cmb['cust_type'].value(res.data[0].cust_type);
				}
			}).done(function(){
				$('.modal-body').show();
				//$("#text_id").hide();
	...
	$('#info-customer-<?php echo $timestamp; ?>').kendoGrid({
		...
		columns: [
			...
			{field:"id",title:"<?php echo lang('tbl_cst_id'); ?>"},
			{field:"customer",title:"<?php echo lang('tbl_cst_name'); ?>"},
			{field:"cp",title:"<?php echo lang('tbl_cst_contact'); ?>",filterable: false, },
			{field:"phone",title:"<?php echo lang('tbl_cst_phone'); ?>",filterable: false, },
			{field:"email",title:"<?php echo lang('tbl_cst_email'); ?>",filterable: false, },
			{field:"alamat",title:"<?php echo lang('tbl_cst_adres'); ?>",filterable: false, },
		]
	});
});
