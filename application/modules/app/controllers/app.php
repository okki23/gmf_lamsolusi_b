...
class App extends MY_Controller {
	...
	function cust_type(){
		$data=array(
			...
			array('id'=>'External', 'text'=>'External')
		);
	...
	function cemical(){
		$calback= $_GET['callback'];
		$data=array(
			array('id'=>'Chemical', 'cemical'=>'Chemical'),
			array('id'=>'Non Chemical', 'cemical'=>'Non Chemical')
		);
	...
	function cmb_requestType_report(){
		...
		foreach($row as $k=>$v){
			...
			if($v =='IMPORT'){
				$res[$k]['id']= 'IMPORT,EXPORT';
				$res[$k]['text']= 'IMPORT EXPORT';
				$no++;
			}
		}
	...
	function curency(){
		$data=array(
			array('id'=>'IDR', 'text'=>'IDR – Indonesia (Rupiah)'),
			array('id'=>'SGD', 'text'=>'SGD – Singapore (Dollar)'),
			array('id'=>'USD', 'text'=>'USD – Us (Dollar)'),
			array('id'=>'EUR', 'text'=>'EUR – Eropa (Euro)')
		);
	...
	function remove_file(){
		...
		if($data !=false || $data !='')
			if(file_exists(FCPATH.'uploads/'.$data))
				unlink(FCPATH.'uploads/'.$data);
